...
class VersionV3:
    ...
    def __init__(
        ...
        self.resolver = RefResolver(
            base_uri=base, referrer=schema, handlers=handlers
        )
        self.error = 0
        ...
        resp_comp["Ok"] = {
            ...
            "content": {
                self.json_content: {
                    "schema": {
                        "$ref": (
                            "#/components/schemas/" + self.resourcedef["name"]
                        )
                    }
                },
                self.yaml_content: {
    ...
    def add_parameters(self):
        ...
        para_comp["PrimaryKeyParm"] = {
            ...
            "in": "path",
            "required": True,
            "schema": {"$ref": "#/components/schemas/primary_key"},
        }
    ...
    def add_basepath(self):
        ...
        ] = utils.jsonschema_compat(resource)
        ...
        if "search" in self.resourcedef:
            for search_by in self.resourcedef["search"]:
                parameters.append(
                    self.openapi["components"]["parameters"][search_by["name"]]
                )
        base_responses = copy.deepcopy(utils.generate_default_response())
        base_responses["200"] = {"$ref": "#/components/responses/Ok_all"}
        opengapigetall = {
            "get": {
    ...
    def findallpaths_in_mhash(
        ...
        self.openapi["components"]["schemas"][propname + "_keys"] = {
            "type": "string",
            "enum": propkeys,
        }
        newschema = {"$ref": "#/components/schemas/" + propname}
        removeschema = {"$ref": ("#/components/schemas/" + propname + "_keys")}
        if parametername not in self.openapi["components"]["parameters"]:
        ...
        for key in propkeys:
            ...
            newschema = {"$ref": "#/components/schemas/" + key}
            reqbody = {
                "required": True,
                "content": {
                    self.yaml_content: {"schema": newschema},
                    self.json_content: {"schema": newschema},
                },
            }
            desc = "insert {0} to a {1}".format(
                propname, self.resourcedef["name"]
            )
            openapimutablehash = {
                "tags": [tagname],
                "description": desc,
                "operationId": (
                    opid + "_" + propname + "_" + key + "_put_" + self.version
                ),
                "parameters": parameters,
                "requestBody": reqbody,
                "responses": self.inresponses,
            }
            self.openapi["paths"][keypath]["put"] = copy.deepcopy(
                openapimutablehash
            )

        # remove
        desc = "delete {0} from a {1}".format(
            propname, self.resourcedef["name"]
        )
        removeparameters = [
            {
        ...
        reqbody = {
            "required": True,
            "content": {
                self.yaml_content: {"schema": jsonpatchdoc},
                self.json_content: {"schema": jsonpatchdoc},
            },
        }
        openapimutablehash["requestBody"] = reqbody
    ...
    def findallpaths_in_proplist(
        ...
        """
        keys = propval["key"]
        itemval = utils.resolve_reference(
            self.resolver, propname, propval["items"], self.schemafile
        )
        if itemval == 1:
            self.error = 1
            return
        ...
        for k in keys:
            items["properties"][k] = {"$ref": "#/components/schemas/" + k}
            ...
            if "required" in value:
                try:
                    ...
                    pass
        self.openapi["components"]["schemas"][propname + "_keys"] = items
        self.openapi["components"]["schemas"][propname + "_value"] = value
        proplist_schema = {
            "$ref": ("#/components/schemas/" + propname + "_keys")
        }
        newschema = {"$ref": "#/components/schemas/" + propname}
        # insert
        insertschema = {
            "$ref": ("#/components/schemas/" + propname + "_value")
        }
        if parametername not in self.openapi["components"]["parameters"]:
            self.openapi["components"]["parameters"][parametername] = {
                "in": "path",
        ...
        insertreqbody = {
            "required": reqbody,
            "content": {
                self.yaml_content: {"schema": insertschema},
                self.json_content: {"schema": insertschema},
            },
        }
        desc = "insert {0} to a {1}".format(propname, self.resourcedef["name"])
        openapiproplist = {
            "tags": [tagname],
            "description": desc,
            "operationId": (
                operationid + "_" + propname[:3] + "_put_" + self.version
            ),
        ...
        self.openapi["paths"][newpath]["put"] = copy.deepcopy(openapiproplist)
        # remove
        desc = "delete {0} from a {1}".format(
            propname, self.resourcedef["name"]
        )
        ...
        openapiproplist["operationId"] = (
            operationid + "_" + propname[:3] + "_delete_" + self.version
        )
        ...
        replaceschema = {
            "type": "array",
            "items": {"$ref": ("#/components/schemas/" + propname)},
        }
        reqbody = {
            "required": propbodyreq,
            "content": {
        ...
        for k in keys:
            if k not in itemval["properties"]:
                ...
                self.error = 1
                return
            self.openapi["components"]["schemas"][k] = itemval["properties"][k]
    ...
    def findallpaths_in_array(
        ...
        self.openapi["paths"][newpath]["put"] = copy.deepcopy(openapiarray)
        # delete
        ...
        self.openapi["paths"][newpath]["delete"] = copy.deepcopy(openapiarray)
        ...
        self.openapi["paths"][replacepath]["put"] = copy.deepcopy(openapiarray)

        # patch
        openapiarray["description"] = "bulk insert/remove part of resource"
        openapiarray["operationId"] = operationid + "_patch"
        oplist = ["insert", "remove"]
        patchschema = {"$ref": "#/components/schemas/" + propname}
        jsonpatchdoc = {
            "type": "array",
            "items": {
                "type": "object",
    ...
    def findallpaths(
        self, propdict, basepath, opid, tagname, parameters, requirelist
    ):
        """
        Add extended paths v3
        """
        for propname, propvalue in propdict.items():
            if utils.check_property_name(propname, self.schemafile):
                self.error = 1
                continue
            propval = utils.resolve_reference(
                self.resolver, propname, propvalue, self.schemafile
            )
            if propval == 1:
                self.error = 1
                continue
            ...
            if "type" not in propval and (
                ...
                self.openapi["paths"][newpath]["put"] = copy.deepcopy(
                    openapibasic
                )
                continue
            if "enum" in propval or (
                propval["type"] in ["string", "boolean", "number", "integer"]
            ):
                self.openapi["components"]["schemas"][propname] = propval
                newpath = basepath + "/" + propname
                self.openapi["paths"][newpath] = dict()
            ...
            if propval["type"] in ["array"]:
                if utils.validate_array_field(
                    self.resolver, propname, propval, self.schemafile
                ):
                    self.error = 1
                    continue
            if propval["type"] in ["propertylist"]:
                if utils.validate_propertylist_field(
                    self.resolver, propname, propval, self.schemafile
                ):
                    self.error = 1
                    continue
        ...
        for verb, val in rpcdef.items():
            ...
            if val["request"]:
                ...
                reqbody = {
                    "required": bodyreq,
                    "content": {
                        self.yaml_content: {"schema": rpcrequest},
                        self.json_content: {"schema": rpcrequest},
                    },
                }
            ...
            responses["200"] = {
                "description": "OK",
                "content": {
                    self.yaml_content: {"schema": rpcresponse},
                    self.json_content: {"schema": rpcresponse},
                },
            }
            responses.update(copy.deepcopy(utils.generate_create_response()))
            rpcpost = {
                "tags": tags,
                "description": desc,
                "operationId": operationid,
    ...
    def write(self):
        ...
        if self.outfmt == "json":
            with open(self.specfile, "w") as outfile:
                _str = json.dumps(
                    self.openapi, indent=4, sort_keys=True, ensure_ascii=False
                )
                outfile.write(_str)
