...
def add_rpcresponses():
    ...
    resp_comp["InternalServerError"] = {"description": "Internal Server Error"}
    resp_comp["ServiceUnavailable"] = {"description": "Service Unavailable"}
    return resp_comp
        ...
        for root, _, files in os.walk(outfiledir):
            for name in files:
                ...
                with open(filename, "w") as outfile:
                    _str = json.dumps(
                        value, indent=4, sort_keys=True, ensure_ascii=False
                    )
                    outfile.write(_str)
...
def validate_array_field(resolver, propname, propval, filename):
    ...
    if "items" not in propval:
        msg = "%s -- items field missing for array in schema file %s"
        _LOG.error(msg, propname, filename)
        error_flag = 1
        return error_flag
    allowed_types = [
    ...
    itemvalue = resolve_reference(
        resolver, propname, propval["items"], filename
    )
    if itemvalue == 1:
        error_flag = 1
        return error_flag
...
def validate_propertylist_field(resolver, propname, propval, filename):
    """
    Validate propertylist entry in schema file
    """
    error_flag = 0
    if "key" not in propval:
        msg = "%s -- missing key field in propertylist in schema file %s"
        _LOG.error(msg, propname, filename)
        error_flag = 1
    if "items" not in propval:
        msg = "%s -- missing items field in schema file %s"
        _LOG.error(msg, propname, filename)
        error_flag = 1
        return error_flag
    itemvalue = resolve_reference(
    ...
    while True:
        ...
        if "$ref" in reference:
            ...
            if _check_valid_ref_file(refname):
                ...
                msg = "Invalid refname %s in schema file %s"
...
def check_basic_fields(resourcedef, filename):
    ...
    """
    for k in ["name", "description", "version", "key"]:
        if k not in resourcedef:
            sys.exit(
                "Mandatory field missing: {0} in schema file {1}".format(
                    k, filename
    ...
    if "type" in resourcedef:
        if resourcedef["type"] != "object":
            sys.exit(
                "Base type should be object in schema file{0}".format(filename)
            )
        if "properties" not in resourcedef:
            sys.exit(
                "Missing field: Properties in schema file {0}".format(filename)
            )
...
def check_property_name(propname, filename):
    ...
    yaml_bool = ["on", "off", "true", "false", "yes", "no"]
...
def check_rpc_definition(rpcverb, rpcval, filename):
    ...
    if "request" not in rpcval or (not rpcval["request"]):
        msg = "%s -- missing request field in rpc section in schema file %s"
        _LOG.error(msg, rpcverb, filename)
        error_flag = 1
    return error_flag
...
def generate_create_response():
    ...
    """
    default_responses = {
        "102": {"$ref": "#/components/responses/Processing"},
...
def generate_default_ok_responses():
    ...
    default_ok_responses["200"] = {"$ref": "#/components/responses/Ok"}
...
def jsonschema_compat(propval):
    ...
    stringpropval = stringpropval.replace(
        '"$ref": "#/definitions/', '"$ref": "#/components/schemas/definitions-'
    )
...
def delete_keys_from_dict(dict_del):
    ...
    """
    lst_keys = ["key", "version", "rpc", "search", "definitions"]
    for k in lst_keys:
        if k in dict_del:
            del dict_del[k]
